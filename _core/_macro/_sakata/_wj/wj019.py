...
股價開盤時走低，但後續盤勢穩定向上，到收盤時抵達相對較高的位置。雖最後有部分賣壓造成
短上影線，但買盤仍然較強。

### 心理面解釋

買盤開始主導行情，使得盤勢開始有較多的上漲，但仍不到足以讓盤勢持續上揚的程度，因此仍
為盤勢不明的階段。
...
def func(market_id:str, **kwargs) -> pd.Series:
    """計算並取得指定市場 ID 中的歷史資料, 每個日期是否有發生紅燭線的序列
...
def plot(**kwargs) -> List[PlotInfo]:
    ...
    try:
        ...
        raise RuntimeError(f"miss argument '{esp.args[0]}' when calling "
                           "'wj019'")
    data = [MokeCandle.make(KType.WHITE_SHORT_EQUAL_SHADOW)]
    data = np.array(data)
    result = [PlotInfo(
        ptype=Ptype.CANDLE,
        title=f"K線",
...
wj019 = Macro(code=code, name=name, description=description, parameters=params,

...
股價有可能是開高走低，也有可能是開低走高，最終收在低於開盤價相對較近的位置。
...
def func(market_id:str, **kwargs) -> pd.Series:
    """計算並取得指定市場 ID 中的歷史資料, 每個日期是否有發生黑色紡錘線的序列
...
def plot(**kwargs) -> List[PlotInfo]:
    ...
    try:
        ...
        raise RuntimeError(f"miss argument '{esp.args[0]}' when calling "
                           "'wj016'")
    data = [MokeCandle.make(KType.BLACK_TINY_LONG_EQUAL_SHADOW)]
    data = np.array(data)
    result = [PlotInfo(
        ptype=Ptype.CANDLE,
        title=f"K線",
...
wj016 = Macro(code=code, name=name, description=description, parameters=params,

...
def func(market_id:str, **kwargs) -> pd.Series:
    """計算並取得指定市場 ID 中的歷史資料, 每個日期是否有發生短紅燭的序列
...
def plot(**kwargs) -> List[PlotInfo]:
    ...
    try:
        ...
        raise RuntimeError(f"miss argument '{esp.args[0]}' when calling "
                           "'wj017'")
    data = [MokeCandle.make(KType.WHITE_TINY_SHORT_EQUAL_SHADOW)]
    data = np.array(data)
    result = [PlotInfo(
        ptype=Ptype.CANDLE,
        title=f"K線",
...
wj017 = Macro(code=code, name=name, description=description, parameters=params,

...
def arg_checker(base_period, short_term_period, long_term_period_1,
    ...
    lower = 1
    keys = ['base_period', 'short_term_period', 'long_term_period_1',
            'long_term_period_2', 'long_term_period_3', 'long_term_period_4',
            'long_term_period_5']
    values = [base_period, short_term_period, long_term_period_1,
              long_term_period_2, long_term_period_3, long_term_period_4,
              long_term_period_5]
    for key, value in zip(keys, values):
        checker = LimitedVariable(lower=lower, upper=MAX_MA_PERIOD)
        if not checker.check(value):
            ret[key] = checker.message
        lower = value + 1
        if lower > MAX_MA_PERIOD:
            lower = None

    checker = LimitedVariable(1, MAX_SMAPLES)
    if not checker.check(short_statistical_duration):
        ret['short_statistical_duration'] = checker.message

    checker = LimitedVariable(short_statistical_duration, MAX_SMAPLES)
    if not checker.check(long_statistical_duration):
        ret['long_statistical_duration'] = checker.message

    checker = LimitedVariable(1, long_statistical_duration)

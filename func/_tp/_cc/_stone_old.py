...
class StoneCandleStick(_CandleStick):
    ...
    def is_ignored_body(self, body_ignore_ratio: float):
        """ignored body."""
        cache_key  = f'is_ignored_body({body_ignore_ratio})'
        if cache_key not in self._cache:
    ...
    def is_long_body(self, long_body_ratio: float):
        """long body."""
        cache_key  = f'is_long_body({long_body_ratio})'
        if cache_key not in self._cache:
    ...
    def is_middle_body(self, long_body_ratio: float, middle_body_ratio: float):
        """middle body."""
        cache_key  = f'is_middle_body({long_body_ratio, middle_body_ratio})'
        if cache_key not in self._cache:
    ...
    def is_strong_positive(self, strong_positive_body_ratio: float):
        """strong positive."""
        cache_key  = f'is_strong_positive({strong_positive_body_ratio})'
        if cache_key not in self._cache:
    ...
    def is_middle_positive(self, strong_positive_body_ratio: float,
        ...
        """middle positive."""
        cache_key  = f'is_middle_positive({strong_positive_body_ratio, middle_positive_body_ratio})'
        if cache_key not in self._cache:
    ...
    def is_strong_negative(self, strong_negative_body_ratio: float):
        """strong negative."""
        cache_key  = f'is_strong_negative({strong_negative_body_ratio})'
        if cache_key not in self._cache:
    ...
    def is_middle_negative(self, strong_negative_body_ratio: float,
        ...
        """middle negative."""
        cache_key  = f'is_middle_negative({strong_negative_body_ratio, middle_negative_body_ratio})'
        if cache_key not in self._cache:
    ...
    def is_ignored_amplitude(self, amplitude_ignore_ratio: float):
        """ignored amplitude."""
        cache_key  = f'is_ignored_amplitude({amplitude_ignore_ratio})'
        if cache_key not in self._cache:
    ...
    def is_ignored_lowershadow(self, shadow_ignore_ratio: float):
        ...
        cache_key  = f'is_ignored_lowershadow({shadow_ignore_ratio})'
        if cache_key not in self._cache:
    ...
    def is_ignored_uppershadow(self, shadow_ignore_ratio: float):
        ...
        cache_key  = f'is_ignored_uppershadow({shadow_ignore_ratio})'
        if cache_key not in self._cache:
    ...
    def is_long_lowershadow(self, long_shadow_ratio: float):
        """long lower-shadow."""
        cache_key  = f'is_long_lowershadow({long_shadow_ratio})'
        if cache_key not in self._cache:
    ...
    def is_long_uppershadow(self, long_shadow_ratio: float):
        """long upper-shadow."""
        cache_key  = f'is_long_uppershadow({long_shadow_ratio})'
        if cache_key not in self._cache:
    ...
    def is_doji(self, body_ignore_ratio: float,
        ...
        """is doji."""
        cache_key  = f'is_doji({body_ignore_ratio, shadow_ignore_ratio})'
        if cache_key not in self._cache:
...
def get_candle(market_id: str, candle_period: int, _cache={}) -> StoneCandleStick:
    if candle_period == 1:
        name = f'{market_id}.CandleStick'
    else:
        name = f'{market_id}.{candle_period}CandleStick'
    if name not in _cache:
        _cache[name] = StoneCandleStick.make(TI.Candle(market_id, candle_period))
    return _cache[name]

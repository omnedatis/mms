...
def check(**kwargs) -> Dict[str, str]:
    ...
    if not _is_positive_integer(min_occurence):
        add_message(results, 'min_occurence', "發生天數必須要為正整數; ")
    ...
    if min_occurence > statistical_duration:
        add_message(results, 'statistical_duration', "發生天數超過判斷天數, 請進行調整; ")
        add_message(results, 'min_occurence', "發生天數超過判斷天數, 請進行調整; ")
    # 均線天期需要小於 2520 

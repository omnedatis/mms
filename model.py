    ...
    for period in PREDICT_PERIODS:
        ...
        for mid in markets:
            ...
            if len(temp) >= MIN_Y_SAMPLES:
                ...
                cur_y.append(ycoder.fit_transform(temp.values[:, -1], outlier=Y_OUTLIER))
...
def model_predict(model: ModelInfo, market: str,
    ...
    for period in PREDICT_PERIODS:
        ...
        while dates[-1] >= next_tdate:
            if controller and not controller.isactive:
                return
            cur_tdate, next_tdate = next_tdate, model.get_next_tdate(next_tdate)
...
def model_update(model_id: str, batch_controller: ThreadController):
    ...
    if not batch_controller.isactive:
        MT_MANAGER.release(model_id)
        return
    exection_id = set_model_execution_start(model_id, ModelExecution.BATCH_PREDICT)
...
def model_create(model: ModelInfo, controller: ThreadController):
    ...
    if not controller.isactive:
        return
    exection_id = set_model_execution_start(model.model_id, ModelExecution.ADD_PREDICT)
    markets = model.markets if model.markets else get_markets()
    ret_buffer = []
    for mid in markets:
        if not controller.isactive:
            return
        ret_buffer.append(model_predict(model, mid, max_len=1, controller=controller))
    if ret_buffer:
        ret = pd.concat(ret_buffer, axis=0)
        ret.index = np.arange(len(ret))
        if controller.isactive:
...
def model_backtest(model: ModelInfo, controller: ThreadController):
    ...
    if not controller.isactive:
        return
    exection_id = set_model_execution_start(model.model_id, ModelExecution.ADD_BACKTEST)
    ...
    while x_data:
        ...
        for mid in list(x_data.keys()):
            ...
            if len(x_data[mid]) <= 0:
                ...
                continue
            # 取得該市場此輪的回測長度
            blen = (x_data[mid].index.values.astype('datetime64[D]') >= tdate).sum()
            if blen <= 0:
                continue
            # 擷取該市場此輪的回測資料
...
def pattern_update(controller: ThreadController):
    ...
    for market in markets:
        ...
        result = fast_concat(result_buffer)
        if not controller.isactive:
            return
        save_pattern_results(market, result)
        cur_ret = pd.DataFrame()
        cur_ret[PatternResultField.VALUE.value] = result.values[-1]
        cur_ret[PatternResultField.DATE.value] = result.index.values.astype('datetime64[D]').tolist()[-1]
        cur_ret[PatternResultField.MARKET_ID.value] = market
        cur_ret[PatternResultField.PATTERN_ID.value] = list(result.columns)
        ...
        result = fast_concat(result_buffer)
        save_future_return(market, result)
    ret = pd.concat(ret_buffer, axis=0)
    if not controller.isactive:
        return
